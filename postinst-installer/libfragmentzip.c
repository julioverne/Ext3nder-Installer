...
#define _impl_PASTE(asaddc,dssdsds) asaddc##dssdsds
#define _impl_CASSERT_LINE(predicate, line, file) \
typedef char _impl_PASTE(assertion_failed_##file##_,line)[2*!!(predicate)-1];

#define assure(asaddc) do{ if ((asaddc) == 0){err=1; goto error;} }while(0)
#define retassure(retcode, asaddc) do{ if ((asaddc) == 0){err=retcode; goto error;} }while(0)
#define safeFree(asaddc) do{ if (asaddc){free(asaddc); asaddc=NULL;} }while(0)
...
fragmentzip_t *fragmentzip_open_extended(const char *url, CURL *mcurl){
    ...
    assure(dbuf = (t_downloadBuffer *)malloc(sizeof(t_downloadBuffer)));
    ...
    curl_easy_setopt(info->mcurl, CURLOPT_CONNECTTIMEOUT, 60L); //60 sec connect timeout
    curl_easy_setopt(info->mcurl, CURLOPT_URL, info->url);
	curl_easy_setopt(info->mcurl, CURLOPT_SSL_VERIFYPEER, 0);
	...
	curl_easy_setopt(info->mcurl, CURLOPT_VERBOSE, 0);
    
	if(strncmp(info->url, "file://", 7) == 0) {
		char path[1024];
		strcpy(path, info->url + 7);
		char* filePath = (char*) curl_easy_unescape(info->mcurl, path, 0,  NULL);
	...
	printf("File Size Is %.02fMB.\n", (float)(info->length / (1024*1024)));
	printf("\n");
	
    //get end of central directory
    assure(dbuf->buf = (char *)malloc(dbuf->size_buf = sizeof(fragmentzip_end_of_cd)));
    
    curl_easy_setopt(info->mcurl, CURLOPT_WRITEFUNCTION, &downloadFunction);
    curl_easy_setopt(info->mcurl, CURLOPT_WRITEDATA, dbuf);
    
    char downloadRange[100];
	//downloadRange = {0};
    ...
    assure(dbuf->buf = (char *)malloc(dbuf->size_buf));
...
int fragmentzip_download_file(fragmentzip_t *info, const char *remotepath, const char *savepath, fragmentzip_process_callback_t callback){
    ...
    retassure(-6,compressed->buf = (char *)malloc(compressed->size_buf = rfile->size_compressed));
    
    bzero(downloadRange,sizeof(downloadRange));
    
    unsigned int start;
	start = (unsigned int)rfile->local_header_offset + sizeof(fragentzip_local_file)-1 + lfile->len_filename + lfile->len_extra_field;
    ...
    retassure(-8,uncompressed = (char *)malloc(rfile->size_uncompressed));
